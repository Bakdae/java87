...
function wdrAccount() {
	...
	$(".wdr-icon-logout").colorbox({href:"../wardrobe/logout.jsp", scrolling:"false"}); //로그아웃 jsp 여는방법 colorbox가
...
function fncAddWardrobe() {
	...
	$("#wdr-create-btn").on("click", function() {
		...
		$.ajax({
			url : "/wardrobe/addWardrobe", //wardrobe 콘트롤러에 
			//addWardrobe /////ajax는 url은 jsp 만 가는것이 아니라 콘트롤러의 메소드에도 갈수 있다.  
			data : {
				"cls_name" : wdrName,
				"user_no" : userNo
			},
