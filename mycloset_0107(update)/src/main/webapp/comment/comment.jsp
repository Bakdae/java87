            ...
            success: function(comment){
              $.get('/comment/getComment?cmtNo='+comment.cmtNo,function(cmt){
                $("#commentList").append("<table class='table table-striped'> <tr><td id ='cmt_nick2'> "+cmt.nick+"</td>"+"<td id='cmt_content2'>"+cmt.content+"</td>"+"<td id='cmt_hrs2'>"+cmt.hrs+"</td>"
                    +"<td class='delCmtBtn'><button    type='submit' class='btn btn-danger btn-xs' style='background-color:white' name="+comment.cmtNo+">삭제</button></td></tr></table>");
  ...
  /* ("<div id='comment_block'> <span id ='cmt_nick'> "+cmt.nick+"</span>"+"<span id='cmt_content'>"+cmt.content+"</span>"
   "<span id='cmt_hrs'>"+cmt.hrs+"</span>"+"<div id='com_delete'><a href='#' id='delCmt2' class='delCmt'>삭제</a></div></div>"); */
      ...
      $(document).on("click",".delCmtBtn",function(){
        ...
        alert(cmtNo);
        $("#commentList").html("");
 /*        $.get('/comment/getCommentList?styleNo='+cmtNo,function(map){
            var row='';      
    $.each(map.commentList,function(index,comment){
     
               row+="<table class='table table-striped'>"
               row+="<tr>";
               row+="<td><span id='cmt_image'><img id='comment_icon' src='../fileUpload/${user.phot_path}'></sapn> <span id='cmt_nick'>  "+comment.nick+"</span>"+"<span id='cmt_hrs'>"+comment.hrs+"</span></td>";
               row+="<td>"+comment.content+"</td>"; 
               row+="<td>"+comment.hrs+"</td>";

               if(comment.userNo == map.user.user_no){
               row+="<td class='delCmtBtn'><button    type='submit' class='btn btn-danger btn-xs' name="+comment.cmtNo+">삭제</button></td>";
               }
